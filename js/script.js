document.querySelector('.menu-burger').onclick = funAddClass;
function funAddClass() {
    document.querySelector('.header__menu').classList.toggle('__active');
    document.querySelector('.menu-burger').classList.toggle('__active');
}

document.querySelectorAll(".ask__question").forEach((elem) => {
  elem.addEventListener("click", function(){
    this.querySelector(".ask__span").classList.toggle("__active");
    this.querySelector("p").classList.toggle("__active");
  });
});


document.querySelector("#selection-input").addEventListener("change", (event) => {
  //alert(event.target.value);
  document.querySelector('.selection__procent').innerHTML = event.target.value;
});
...
function goTop() {
  ...
  if (window.pageYOffset > 0) {
    // скроллим наверх
    window.scrollBy(0, -205); // второй аргумент - скорость
